# -*- coding: utf-8 -*-
# @Time    : 2020/11/6 3:36 下午 (Original) | 2026/2/24 (Modified)
# @Author  : Haohe Liu (Original) | Concrete Eavesdrop System 3022234317@tju.edu.cn (Modified)
...
# ============================================================================
# Windows / SoX 兼容层：优先用系统级 augment，失败则无缝切换到 third_party
# ============================================================================
try:
    import augment
    import torchaudio
    _ = torchaudio.sox_effects.effect_names()
except (RuntimeError, ImportError, OSError, AttributeError):
    _root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    _tp = os.path.join(_root, "third_party")
    if _tp not in sys.path:
        sys.path.insert(0, _tp)
    
    import augment  # 从 third_party/augment 加载
    
    # 【把下面这段全删了或者注释掉！】
    # warnings.simplefilter('once', UserWarning)
    # warnings.warn(
    #     "\n" + "="*60 + "\n"
    #     "[环境提示] 系统原生 SoX 后端不可用 (常见于 Windows)。\n"
    #     "已自动切换到纯 PyTorch/Scipy 的替代实现，不影响模型训练。\n"
    #     + "="*60,
...
class MagicalEffects:
    ...
    def pick_out_effects(self, effects: dict, type_=None):
        if type_ is None:
            raise ValueError("You should specify a type you wanna pick out.")
        effect_chain = []
        for key in effects.keys():
            if self.ps.effects_type[key] == type_:
                effect_chain.append(key)
        return effect_chain
    ...
    def generate_effect_chain(self, effects, audio_shape, sample_rate=44100):
        ...
        for key in items:
            if key == 'fade':
                chain = self.fade(chain, effects['fade'], audio_len=audio_shape[0] / sample_rate)
            else:
                raise ValueError("Bad parameter", key, "in probability setting")
        items = self.pick_out_effects(effects=effects, type_=1)
        for key in items:
            if key == 'pitch':
                chain = self.pitch(chain, effects['pitch'], sample_rate=sample_rate)
            elif key == 'tempo':
                chain = self.tempo(chain, effects['tempo'])
            elif key == 'speed':
                chain = self.speed(chain, effects['speed'])
            elif key == 'treble':
                chain = self.treble(chain, effects['treble'])
            elif key == 'bass':
                chain = self.bass(chain, effects['bass'])
            elif key == 'tremolo':
                chain = self.tremolo(chain, effects['tremolo'])
            elif key == 'clip':
                chain = self.clip(chain, effects['clip'])
            elif key == 'reverb_freeverb':
                chain = self.reverb_freeverb(chain, effects['reverb_freeverb'])
            elif key == 'low_pass':
                chain = self.low_pass(chain, effects['low_pass'])
            elif key == 'high_pass':
                chain = self.high_pass(chain, effects['high_pass'])
            elif key == 'reverse':
                chain = self.reverse(chain, effects['reverse'])
            else:
                raise ValueError("Bad parameter", key, "in probability setting")
        return chain
    ...
    def reverb_freeverb(self, chain, params):
        if len(params[1]) != 3:
            raise ValueError("Bad parameters in reverb_freeverb()")
